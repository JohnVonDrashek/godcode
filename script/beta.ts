...
async function fix(pr: PR, files: string[]) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with holycode...`)
  const prompt = [
  ...
  try {
    await $`holycode run -m opencode/gpt-5.3-codex ${prompt}`
  } catch (err) {
    console.log(`  holycode failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with holycode")
  return true
}
