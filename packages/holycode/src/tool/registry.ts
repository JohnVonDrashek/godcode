...
import { type ToolContext as PluginToolContext, type ToolDefinition } from "@crusadesoft/plugin"
import z from "zod"
...
export namespace ToolRegistry {
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@holycode/ToolRegistry") {}
  ...
  export const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      const cache = yield* InstanceState.make<State>(
        Effect.fn("ToolRegistry.state")(function* (ctx) {
          ...
          yield* Effect.promise(async () => {
            const matches = await Config.contentDirectories().then((dirs) =>
              dirs.flatMap((dir) =>
      ...
      async function all(custom: Tool.Info[]): Promise<Tool.Info[]> {
        ...
        const question = ["app", "cli", "desktop"].includes(Flag.HOLYCODE_CLIENT) || Flag.HOLYCODE_ENABLE_QUESTION_TOOL
        ...
        return [
          ...
          ...(Flag.HOLYCODE_EXPERIMENTAL_LSP_TOOL ? [LspTool] : []),
            ...
            allTools
              .filter((tool) => {
                ...
                if (tool.id === "codesearch") {
                  return model.providerID === ProviderID.opencode || Flag.HOLYCODE_ENABLE_EXA
                }
