...
export namespace Skill {
  ...
  const HOLYCODE_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
  ...
  const create = (discovery: Discovery.Interface, directory: string, worktree: string): Cache => {
    ...
    const load = async () => {
      ...
      await scan(state, DotHolycode.root(), "skills/**/SKILL.md", { kind: "builtin" })

      if (!Flag.HOLYCODE_DISABLE_EXTERNAL_SKILLS) {
      ...
      for (const dir of await Config.directories()) {
        await scan(state, dir, HOLYCODE_SKILL_PATTERN, { kind: "config" })
      }
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@holycode/Skill") {}
