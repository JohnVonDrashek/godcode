...
import { NamedError } from "@crusadesoft/util/error"
...
import { DotHolycode } from "@/dot-holycode"
...
export namespace Config {
  ...
  export function managedConfigDir() {
    return process.env.HOLYCODE_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
  }
  ...
  export const state = Instance.state(async () => {
    ...
    // Config loading order (low -> high precedence): https://holycode.ai/docs/config#precedence-order
    // 1) Remote .well-known/holycode (org defaults)
    // 2) Global config (~/.config/holycode/holycode.json{,c})
    // 3) Custom config (HOLYCODE_CONFIG)
    ...
    // 6) Inline config (HOLYCODE_CONFIG_CONTENT)
    ...
    for (const [key, value] of Object.entries(auth)) {
      if (value.type === "wellknown") {
        ...
        log.debug("fetching remote config", { url: `${url}/.well-known/holycode` })
        const response = await fetch(`${url}/.well-known/holycode`)
        if (!response.ok) {
        ...
        if (!remoteConfig.$schema) remoteConfig.$schema = "https://holycode.ai/config.json"
        result = mergeConfigConcatArrays(
          result,
          await load(JSON.stringify(remoteConfig), {
            dir: path.dirname(`${url}/.well-known/holycode`),
            source: `${url}/.well-known/holycode`,
          }),
        )
    ...
    if (Flag.HOLYCODE_CONFIG) {
      result = mergeConfigConcatArrays(result, await loadFile(Flag.HOLYCODE_CONFIG))
      log.debug("loaded custom config", { path: Flag.HOLYCODE_CONFIG })
    }

    // Project config overrides global and remote config.
    if (!Flag.HOLYCODE_DISABLE_PROJECT_CONFIG) {
    ...
    const content = [DotHolycode.root(), ...directories]

    // .holycode directory config overrides (project and global) config sources.
    if (Flag.HOLYCODE_CONFIG_DIR) {
      log.debug("loading config from HOLYCODE_CONFIG_DIR", { path: Flag.HOLYCODE_CONFIG_DIR })
    }

    const deps = []

    for (const dir of unique(content)) {
      if (dir !== DotHolycode.root() && (dir.endsWith(".holycode") || dir === Flag.HOLYCODE_CONFIG_DIR)) {
    ...
    if (process.env.HOLYCODE_CONFIG_CONTENT) {
      result = mergeConfigConcatArrays(
        result,
        await load(process.env.HOLYCODE_CONFIG_CONTENT, {
          dir: Instance.directory,
          source: "HOLYCODE_CONFIG_CONTENT",
        }),
      )
      log.debug("loaded custom config from HOLYCODE_CONFIG_CONTENT")
    }
    ...
    if (active?.active_org_id) {
      try {
        ...
        if (token) {
          process.env["HOLYCODE_CONSOLE_TOKEN"] = token
          Env.set("HOLYCODE_CONSOLE_TOKEN", token)
        }
    ...
    if (Flag.HOLYCODE_PERMISSION) {
      result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.HOLYCODE_PERMISSION))
    }
    ...
    if (Flag.HOLYCODE_DISABLE_AUTOCOMPACT) {
      result.compaction = { ...result.compaction, auto: false }
    }
    if (Flag.HOLYCODE_DISABLE_PRUNE) {
  ...
  export async function installDependencies(dir: string) {
    ...
    json.dependencies = {
      ...json.dependencies,
      [Installation.PLUGIN_PACKAGE]: targetVersion,
    }
    ...
    await BunProc.run(
      ...
      if (err instanceof Process.RunFailedError) {
        ...
        if (Flag.HOLYCODE_STRICT_CONFIG_DEPS) {
  ...
  export async function needsInstall(dir: string) {
    ...
    const depVersion = dependencies[Installation.PLUGIN_PACKAGE]
    if (!depVersion) return true

    const targetVersion = Installation.isLocal() ? "latest" : Installation.VERSION
    if (targetVersion === "latest") {
      const isOutdated = await PackageRegistry.isOutdated(Installation.PLUGIN_PACKAGE, depVersion, dir)
      if (!isOutdated) return false
      log.info("Cached version is outdated, proceeding with install", {
        pkg: Installation.PLUGIN_PACKAGE,
   ...
   * 2. Local holycode.json
   * 3. Global plugin/ directory
   * 4. Global holycode.json
   *
  ...
  export const Info = z
    .object({
      ...
      command: z
        ...
        .optional()
        .describe("Command configuration, see https://holycode.ai/docs/commands"),
      ...
      agent: z
        ...
        .catchall(Agent)
        .optional()
        .describe("Agent configuration, see https://holycode.ai/docs/agents"),
      provider: z
    ...
    if (existsSync(legacy)) {
      await import(pathToFileURL(legacy).href, {
        ...
        .then(async (mod) => {
          ...
          result["$schema"] = "https://holycode.ai/config.json"
  ...
  async function load(text: string, options: { path: string } | { dir: string; source: string }) {
    ...
    if (parsed.success) {
      if (!parsed.data.$schema && isFile) {
        parsed.data.$schema = "https://holycode.ai/config.json"
  ...
  function projectStateConfigFile() {
    const dir = path.join(root(), ".holycode")
    const files = ConfigPaths.fileInDirectory(dir, "holycode")
    for (const file of files) {
      if (existsSync(file)) return file
    }
    return projectConfigFile().endsWith(".jsonc") ? files[0] : files[1]
  }

  export async function updateProject(config: Info) {
    const filepath = projectStateConfigFile()
    await fs.mkdir(path.dirname(filepath), { recursive: true })
    const legacy = projectConfigFile()
    const source = await (async () => {
      const text = await Filesystem.readText(filepath).catch((err: any) => {
        if (err.code === "ENOENT") return undefined
        throw new JsonError({ path: filepath }, { cause: err })
      })
      if (text !== undefined) return { path: filepath, text }
      const fallback = await Filesystem.readText(legacy).catch((err: any) => {
        if (err.code === "ENOENT") return undefined
        throw new JsonError({ path: legacy }, { cause: err })
      })
      if (fallback !== undefined) return { path: legacy, text: fallback }
      return { path: filepath, text: "{}" }
    })()

    const next = await (async () => {
      if (!filepath.endsWith(".jsonc")) {
        const merged = mergeDeep(parseConfig(source.text, source.path), config)
        merged.$schema ??= "https://holycode.ai/config.json"
        await Filesystem.writeJson(filepath, merged)
        return merged
      }

      const updated = patchJsonc(source.text, config)
  ...
  export async function contentDirectories() {
    return [DotHolycode.root(), ...(await directories())]
  }
}
Filesystem.write
Filesystem.write
