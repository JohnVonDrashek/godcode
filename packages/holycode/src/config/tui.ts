...
export namespace TuiConfig {
  ...
  function customPath() {
    return Flag.HOLYCODE_TUI_CONFIG
  }

  const state = Instance.state(async () => {
    let projectFiles = Flag.HOLYCODE_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.HOLYCODE_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!dir.endsWith(".holycode") && dir !== Flag.HOLYCODE_CONFIG_DIR) continue
