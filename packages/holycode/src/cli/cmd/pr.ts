...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run HolyCode",
  builder: (yargs) =>
  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
          ...
          if (prInfoText.trim()) {
            ...
            if (prInfo && prInfo.body) {
              ...
              if (sessionMatch) {
                ...
                const importResult = await Process.text(["holycode", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting HolyCode...")
        UI.println()

        const sessionArgs = sessionId ? ["-s", sessionId] : []
        const child = Process.spawn(["holycode", ...sessionArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        ...
        if (code !== 0) throw new Error(`holycode exited with code ${code}`)
      },
    })
  },
})
