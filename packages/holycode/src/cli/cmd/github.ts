...
const AGENT_USERNAME = "holycode-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/holycode.yml"
      ...
      async fn() {
        {
          ...
          function printNextSteps() {
            ...
            prompts.outro(
              [
                ...
                step2,
                "",
                "    3. Go to a GitHub issue and comment `/holycode summarize` to see the agent in action",
                "",
                "   Learn more about the GitHub agent - https://github.com/crusadesoft/holycode/tree/dev/github",
              ].join("\n"),
            )
          }
          ...
          async function installGitHubApp() {
            ...
            // Open browser
            const url = "https://github.com/apps/holycode-agent"
            const command =
            ...
            async function getInstallation() {
              return await fetch(
                `https://api.holycode.ai/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`,
              )
                .then((res) => res.json())
...
jobs:
  holycode:
    if: |
      ...
      startsWith(github.event.comment.body, '/hc') ||
      contains(github.event.comment.body, ' /holycode') ||
      startsWith(github.event.comment.body, '/holycode')
    ...
    steps:
      ...
      - name: Run HolyCode
        uses: crusadesoft/holycode/github@latest${envStr}
        with:
...
export const GithubRunCommand = cmd({
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.holycode.ai" : "https://holycode.ai"
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://api.holycode.ai"
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/holycode,/hc")
          .split(",")
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("holycode-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `holycode/${type}-${hex}-${timestamp}`
        }
        return `holycode/${type}${issueId}-${timestamp}`
      }
      ...
      function footer(opts?: { image?: boolean }) {
        const image = (() => {
          ...
          return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/holycode-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
        })()
        const shareUrl = shareId ? `[holycode session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the holycode infrastructure after your response",
