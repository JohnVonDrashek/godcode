...
import { createHolycodeClient, type Message, type HolycodeClient, type ToolPart } from "@crusadesoft/sdk/v2"
...
export const RunCommand = cmd({
  ...
  handler: async (args) => {
    ...
    async function session(sdk: HolycodeClient) {
    ...
    async function execute(sdk: HolycodeClient) {
    ...
    await bootstrap(process.env.HOLYCODE_CWD || process.cwd(), async () => {
      const fetchFn = (async (input: RequestInfo | URL, init?: RequestInit) => {
        const request = new Request(input, init)
        return Server.Default().fetch(request)
      }) as typeof globalThis.fetch
      const sdk = createHolycodeClient({ baseUrl: "http://holycode.internal", fetch: fetchFn })
      await execute(sdk)
    })
  },
})
