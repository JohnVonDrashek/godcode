...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall HolyCode and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall HolyCode")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: `npm uninstall -g ${Installation.PACKAGE}`,
      pnpm: `pnpm uninstall -g ${Installation.PACKAGE}`,
      bun: `bun remove -g ${Installation.PACKAGE}`,
      yarn: `yarn global remove ${Installation.PACKAGE}`,
      brew: "brew uninstall holycode",
      choco: "choco uninstall holycode",
      scoop: "scoop uninstall holycode",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "holycode", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using HolyCode!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# holycode") || content.includes(".holycode/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".holycode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".holycode/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".holycode"))
    ) {
      continue
    }
