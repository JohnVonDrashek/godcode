    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.outro("Add servers with: holycode mcp add")
          return
        }
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info(
            "Remote MCP servers support OAuth by default. Add a remote server in .holycode/holycode.json:",
          )
          prompts.log.info(`
  "mcp": {
    "my-server": {
...
async function resolveConfigPath(baseDir: string, global = false) {
  ...
  // Default to hidden project config if none exist
  return candidates[0]
}
      ...
      async fn() {
        ...
        if (type === "local") {
          const command = await prompts.text({
            ...
            placeholder: "e.g., holycode x @modelcontextprotocol/server-filesystem",
            validate: (x) => (x && x.length > 0 ? undefined : "Required"),
          })
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "holycode-debug", version: Installation.VERSION },
              },
              id: 1,
            }),
          })
