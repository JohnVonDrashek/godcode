  ...
  async function prepare(input: StreamInput): Promise<Prepared> {
    ...
    const finalHeaders = {
      ...(input.model.providerID.startsWith("opencode")
        ? {
            "x-holycode-project": Instance.project.id,
            "x-holycode-session": input.sessionID,
            "x-holycode-request": input.user.id,
            "x-holycode-client": Flag.HOLYCODE_CLIENT,
          }
        : {
            "User-Agent": `holycode/${Installation.VERSION}`,
          }),
      ...input.model.headers,
      ...headers,
    }
