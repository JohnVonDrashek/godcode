...
import { Slug } from "@crusadesoft/util/slug"
...
export namespace SessionPrompt {
  ...
  async function insertReminders(input: {
    ...
    const prev = assistantMessage?.info.agent
    const ctx = {
      session: input.session,
      persist: input.persist,
      directory: Instance.directory,
      worktree: Instance.worktree,
      plan: Session.plan(input.session),
    }
    ...
    if (prev && prev !== input.agent.name) {
      await push(await Agent.leave(prev, ctx))
    }

    if (prev !== input.agent.name) {
      await push(await Agent.enter(input.agent.name, ctx))
    }
