...
test("plan agent denies edits except .holycode/plans/*", async () => {
  await using tmp = await tmpdir()
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      ...
      expect(Permission.evaluate("edit", ".holycode/plans/foo.md", plan!.permission).action).toBe("allow")
    },
  })
})

test("code agents from .holycode/agents can patch and hook runtime behavior", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const file = path.join(dir, ".holycode", "agents", "coder.ts")
...
`,
  ...
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      const agent = await Agent.get("coder")
      ...
      const input = {
        ...
        directory: tmp.path,
        worktree: tmp.path,
        plan: path.join(tmp.path, ".holycode", "plans", "plan.md"),
      }

      expect(await Agent.enter("coder", input)).toBe("enter hook")
      expect(await Agent.leave("coder", input)).toBe("leave hook")
    },
  })
})

test("refactor agent is switchable and allows planning questions", async () => {
  await using tmp = await tmpdir()
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      const refactor = await Agent.get("refactor")
      expect(refactor).toBeDefined()
      expect(refactor?.mode).toBe("primary")
      expect(refactor?.native).toBe(true)
      expect(refactor?.switch).toBe(true)
      expect(evalPerm(refactor, "question")).toBe("allow")
      expect(evalPerm(refactor, "plan_enter")).toBe("allow")
      expect(refactor?.prompt).toContain("independent-research")
    },
  })
})

test("architect agent is switchable and stays architecture-focused", async () => {
  await using tmp = await tmpdir()
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      ...
      expect(architect?.switch).toBe(true)
      expect(evalPerm(architect, "question")).toBe("allow")
      expect(evalPerm(architect, "plan_enter")).toBe("allow")
      expect(architect?.prompt).toContain("independent-research")
      expect(architect?.prompt).toContain("architectural")
    },
  })
})
...
test("skill directories are allowed for external_directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".holycode", "skill", "perm-skill")
      await Bun.write(
...
`,
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        const build = await Agent.get("build")
        const skillDir = path.join(tmp.path, ".holycode", "skill", "perm-skill")
