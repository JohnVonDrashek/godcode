...
test("loads tui config with the same precedence order as server config paths", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(
        path.join(dir, ".holycode", "tui.json"),
...
test("migrates tui-specific keys from holycode.json when tui.json does not exist", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const server = JSON.parse(await Filesystem.readText(path.join(tmp.path, "holycode.json")))
      expect(server.theme).toBeUndefined()
      expect(server.keybinds).toBeUndefined()
      expect(server.tui).toBeUndefined()
      expect(await Filesystem.exists(path.join(tmp.path, "holycode.json.tui-migration.bak"))).toBe(true)
...
test("skips migration when holycode.jsonc is syntactically invalid", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(await Filesystem.exists(path.join(tmp.path, "holycode.jsonc.tui-migration.bak"))).toBe(false)
      const source = await Filesystem.readText(path.join(tmp.path, "holycode.jsonc"))
...
test("skips migration when tui.json already exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "holycode.json"), JSON.stringify({ theme: "legacy" }, null, 2))
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(await Filesystem.exists(path.join(tmp.path, "holycode.json.tui-migration.bak"))).toBe(false)
    },
  })
})
...
test("continues loading tui config when legacy source cannot be stripped", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "holycode.json"), JSON.stringify({ theme: "readonly-theme" }, null, 2))
    },
  })

  const source = path.join(tmp.path, "holycode.json")
...
test("migration backup preserves JSONC comments", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      await TuiConfig.get()
      const backup = await Filesystem.readText(path.join(tmp.path, "holycode.jsonc.tui-migration.bak"))
...
test("migrates legacy tui keys across multiple holycode.json levels", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const nested = path.join(dir, "apps", "client")
      await fs.mkdir(nested, { recursive: true })
      await Bun.write(path.join(dir, "holycode.json"), JSON.stringify({ theme: "root-theme" }, null, 2))
      await Bun.write(path.join(nested, "holycode.json"), JSON.stringify({ theme: "nested-theme" }, null, 2))
    },
  })
...
test("does not derive tui path from OPENCODE_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(path.join(customDir, "holycode.json"), JSON.stringify({ model: "test/model" }))
      await Bun.write(path.join(customDir, "tui.json"), JSON.stringify({ theme: "should-not-load" }))
      process.env.OPENCODE_CONFIG = path.join(customDir, "holycode.json")
    },
  })
...
test("loads .holycode/tui.json", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.mkdir(path.join(dir, ".holycode"), { recursive: true })
      await Bun.write(path.join(dir, ".holycode", "tui.json"), JSON.stringify({ diff_style: "stacked" }, null, 2))
    },
  })
