...
const managedConfigDir = process.env.HOLYCODE_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "holycode.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "holycode.json") {
...
test("ignores legacy tui keys in holycode config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://holycode.ai/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "holycode.json"))
...
test("resolves env templates in account config with account token", async () => {
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        const config = await Config.get()
        expect(config.provider?.["holycode"]?.options?.apiKey).toBe("st_test_token")
      },
    })
  } finally {
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "holycode.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .holycode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const holycodeDir = path.join(dir, ".holycode")
      await fs.mkdir(holycodeDir, { recursive: true })
      const agentDir = path.join(holycodeDir, "agent")
...
test("loads agents from .holycode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const holycodeDir = path.join(dir, ".holycode")
      await fs.mkdir(holycodeDir, { recursive: true })

      const agentsDir = path.join(holycodeDir, "agents")
...
test("loads commands from .holycode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const holycodeDir = path.join(dir, ".holycode")
      await fs.mkdir(holycodeDir, { recursive: true })

      const commandDir = path.join(holycodeDir, "command")
...
test("loads commands from .holycode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const holycodeDir = path.join(dir, ".holycode")
      await fs.mkdir(holycodeDir, { recursive: true })

      const commandsDir = path.join(holycodeDir, "commands")
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "holycode.json"),
        JSON.stringify({ $schema: "https://holycode.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "holycode.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .holycode config
      const projectDir = path.join(dir, "project")
      const holycodeDir = path.join(projectDir, ".holycode")
      await fs.mkdir(holycodeDir, { recursive: true })
      ...
      // Local .holycode config with different plugins
      await Filesystem.write(
        path.join(holycodeDir, "holycode.json"),
        JSON.stringify({
          $schema: "https://holycode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .holycode config with some overlapping plugins
      await Filesystem.write(
        path.join(holycodeDir, "holycode.json"),
        JSON.stringify({
          $schema: "https://holycode.ai/config.json",
...
// Note: preload.ts sets HOLYCODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .holycode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .holycode directory config enables it
      const holycodeDir = path.join(dir, ".holycode")
      await fs.mkdir(holycodeDir, { recursive: true })
      await Filesystem.write(
        path.join(holycodeDir, "holycode.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/holycode")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/holycode")
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.holycode/plugin/oh-my-opencode.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.holycode/plugin/oh-my-opencode.js")
  })
  ...
  test("local plugin directory overrides global holycode.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const holycodeDir = path.join(projectDir, ".holycode")
        const pluginDir = path.join(holycodeDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("OPENCODE_DISABLE_PROJECT_CONFIG", () => {
  ...
  test("skips project .holycode/ directories when flag is set", async () => {
    const originalEnv = process.env["OPENCODE_DISABLE_PROJECT_CONFIG"]
    process.env["OPENCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .holycode directory with a command
          const holycodeDir = path.join(dir, ".holycode", "command")
          await fs.mkdir(holycodeDir, { recursive: true })
          await Filesystem.write(path.join(holycodeDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .holycode should NOT be in directories list
