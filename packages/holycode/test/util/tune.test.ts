...
describe("tune", () => {
  test("default tune includes core defaults", () => {
    const text = Tune.prompt(Tune.DEFAULT)

    expect(text).toContain("You are HolyCode, the best coding agent on the planet.")
    expect(text).toContain("## Editing constraints")
    expect(text).toContain("## Tool usage")
    expect(text).toContain("## Frontend tasks")
    expect(text).toContain("## Presenting your work and final message")
...
`,
    ...
    const prev = process.env.HOLYCODE_TUNE_DIR
    process.env.HOLYCODE_TUNE_DIR = tmp.extra
    Tune.reload()
    ...
    try {
      ...
      if (prev === undefined) delete process.env.HOLYCODE_TUNE_DIR
      else process.env.HOLYCODE_TUNE_DIR = prev
      Tune.reload()
    }
  })
})
