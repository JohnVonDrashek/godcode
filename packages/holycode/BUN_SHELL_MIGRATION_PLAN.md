...
Replace runtime Bun shell template-tag usage in `packages/holycode/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/holycode/src` is removed except:
