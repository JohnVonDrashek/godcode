...
**Only** add if the issue mentions "zen" or "holycode zen" or "holycode black".

If the issue doesn't have "zen" or "holycode black" in it then don't add zen label

#### core

Use for core server issues in `packages/holycode/`, excluding `packages/holycode/src/cli/cmd/tui/`.
...
TUI (`packages/holycode/src/cli/cmd/tui/...`):
...
Core (`packages/holycode/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for holycode server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
